...
<v-container>
    ...
    <v-row>
        <v-text-field label="Name" placeholder="Thundertree Cultists" hint="Enter the name of your encounter. This will help you identify it later on." clearable counter="25" v-model="name"></v-text-field>
    </v-row>
    <v-row>
        <v-col>
    ...
    <v-row>
        <v-col>
            <v-chip-group v-for="(player, index) in players" :key="player.name">
                <v-chip close @click:close="removePlayerFromList(index)" color="blue">{{player.name}} Level: {{player.level}}</v-chip>
            </v-chip-group>
        </v-col>
    </v-row>
    <v-row>
